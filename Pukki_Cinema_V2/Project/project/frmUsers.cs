...
namespace project
{
    public partial class frmUsers : Form
    {
                ...
                if (textbx_Username.Text != "" && txt_password.Text != "" && rdoFlag)
                {
                   /* if (textbx_Username.Text != "")
                    ...
                    if(txt_password.Text != "")
                        errorProvider1.SetError(textbx_Username, "Please enter the password to be added");
                    if (rdoFlag)
        ...
        {
            (dataGridView1.DataSource as DataTable).DefaultView.RowFilter = string.Format("Username LIKE '%{}%'", txtSearch.Text);
    ...
    private void btn_previous_Click(object sender, EventArgs e)
        ...
        private void cbUserID_SelectedIndexChanged(object sender, EventArgs e, string username, int Users_ID)
        {
            conn.Open();
            username = (string)(dataGridView1.CurrentRow.Cells[1].Value.ToString());
            com = new SqlCommand("SELECT Username FROM USERS WHERE Users_ID=@userID", conn);
            //sql = $"SELECT Users_ID FROM USERS WHERE username " = '{username}'";
            com = new SqlCommand(sql, conn);
            adap.SelectCommand = com;
            Users_ID = (int)com.ExecuteScalar();
            adap.Fill(ds, "USERS");
